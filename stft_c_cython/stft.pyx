...
cdef extern from "c_code/stft.c":
    double* stft(double *wav_data, int samples, int windowSize, int hop_size,\
                 double *magnitude, double *frequencies, int sample_freq, int length)
...
cpdef play(audData, rate,windowSize,hopSize):
    ...
    #compute the stft
    stft(&audData_view[0], n_elements, windowSize, hopSize,&magnitude[0], &frequencies[0],rate, length)
    ...
    return  new_array, frequencies_array
